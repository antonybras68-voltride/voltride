...
router.post('/', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    const result = await pool.query(`
      ...
      VALUES ($1, $2, $3, $4, $5, $6)
      RETURNING *
    `, [code, name, address, phone, email, opening_hours]);
...
router.put('/:id', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    // Vérifier si le nouveau code est déjà utilisé par une autre agence
    if (code) {
    ...
    const result = await pool.query(`
      ...
      WHERE id = $7
      RETURNING *
    `, [code, name, address, phone, email, opening_hours, id]);
...
router.delete('/:id', authMiddleware, async (req, res) => {
  ...
  try {
    // Vérifier s'il y a des véhicules ou locations liés
    ...
    if (parseInt(vehiclesCheck.rows[0].count) > 0) {
      return res.status(400).json({ error: 'No se puede eliminar: hay vehículos asociados a esta agencia' });
    }
    ...
    if (parseInt(rentalsCheck.rows[0].count) > 0) {
      return res.status(400).json({ error: 'No se puede eliminar: hay alquileres asociados a esta agencia' });
    }
