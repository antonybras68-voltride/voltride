...
router.post('/', authMiddleware, async (req, res) => {
  try {
    ...
    const result = await pool.query(`
      ...
      RETURNING *
    `, [code, type, brand, model, color, daily_rate, deposit || 0, status || 'available', agency_id, notes]);
...
router.put('/:id', authMiddleware, async (req, res) => {
  try {
    ...
    const result = await pool.query(`
      ...
      WHERE id = $11
      RETURNING *
    `, [code, type, brand, model, color, daily_rate, deposit, status, agency_id, notes, id]);
...
// GET /api/vehicles/stats/summary - Estadísticas de vehículos
